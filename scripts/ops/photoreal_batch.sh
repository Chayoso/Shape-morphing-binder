...
SURFACE=   # docs/method.md 10.12: the outer-layer Poisson surface; mc = the v8 level set
TRACK=             # docs/method.md 10.15 (2026-09-22): the surface is tracked with the material; TRACK=0 = per-frame
...
for T in "$@"; do
  ...
  CUDA_VISIBLE_DEVICES=$GPU $PY scripts/render_photoreal.py --npz $OUT/${PFX}_${T}_${ARM}.npz \
    --out /_photoreal.mp4 --res 720 --stride 3 --surface  197609 [ "" = 1 ] && echo --track ) --label "sphere -> $T, $NLAB, photoreal" \
