...
# scripts/probes/surface_gt.py against the true mesh. Usage: surface_test.sh <gpu> <run> <frame>
...
LOG=$D/log.txt; : > $LOG
declare -A FLAGS=( [mc]="" [pca]="--kernel pca" [poisson]="--surface poisson" [imls]="--surface imls"
                   [surfel]="--surface surfel" [bilateral]="--post bilateral" )
for C in mc pca poisson imls surfel bilateral; do
  for F in -2 $FR; do
    ...
    CUDA_VISIBLE_DEVICES=$GPU $PY scripts/render_photoreal.py --npz $OUT/${RUN}_${ARM}.npz \
      ...
      2>&1 | grep -v "^\[photoreal\] frame\|Warning" | tail -6 | tee -a $LOG
  done
done
echo "== probe $(date +%H:%M:%S)" | tee -a $LOG
$PY scripts/probes/surface_gt.py $OUT/$RUN.json $D/mc_target.ply $D/pca_target.ply $D/poisson_target.ply \
    $D/imls_target.ply $D/surfel_target.ply $D/bilateral_target.ply 2>&1 | tee -a $LOG
