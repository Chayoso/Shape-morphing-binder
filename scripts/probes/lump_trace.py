...
def main():
    ...
    h_cell = dx / sp
    k_cell = int(round(24 * (h_cell / 2.0) ** 2))
    ...
    for i in idx:
        ...
        r2, _ = plane_residual(x, m, n, sp, k=24, h_sp=2.0)
        rc, _ = plane_residual(x, m, n, sp, k=k_cell, h_sp=h_cell)
        rows.append((i, float(np.sqrt((r2 ** 2).mean())), float(np.sqrt((rc ** 2).mean())), float(m.mean())))
        print(f"frame {i:5d}: rms 2sp {rows[-1][1]:.3f}  rms cell {rows[-1][2]:.3f} sp  (layer {rows[-1][3] * 100:.1f} %)")
    a = np.array(rows)
    print(f"spacing {sp:.4f} wu, cell {dx:.4f} wu = {h_cell:.2f} sp (K {k_cell}); mean over the morph: 2sp {a[:, 1].mean():.3f}, "
          f"cell {a[:, 2].mean():.3f} sp; end: 2sp {a[-1, 1]:.3f}, cell {a[-1, 2]:.3f}; peak cell {a[:, 2].max():.3f} at frame {int(a[a[:, 2].argmax(), 0])}")
    if png:
        import matplotlib
        matplotlib.use("Agg")
        ...
        ax.plot(a[:, 0], a[:, 2], label=f"at one MPM cell = {h_cell:.1f} spacings (K {k_cell})", color="#B5442E")
