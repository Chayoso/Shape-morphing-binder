...
def arm_config(arm: str, args) -> PipelineConfig:
    cfg = PipelineConfig(T=args.T, iters=args.iters, animations=args.animations,
                         ...
                         persistent_rest_volume=not args.legacy_recompute_volumes,
                          ...
                          pace_lead_from=args.pace_lead_from, ctrl_taper_sp=args.ctrl_taper_sp, pace_lead_gov=args.pace_lead_gov)
    if arm == "phys":
        cfg.lambda_auto = 0.0
    elif arm == "render":
...
def main():
    ...
    ap.add_argument("--pace_lead_gov", action="store_true", help="tested-candidate lead governor: half/double the lead per window, kept only if progress to the final images and the moving set thinning do not worsen (config.pace_lead_gov; P289)")
    ...
    ap.add_argument("--pace_front_pts", action="store_true", help="the paced target as a front at the particle scale (config.pace_front_pts)")
    ap.add_argument("--pace_cap", action="store_true", help="with a front: the paced grid capped at the target's cell mass (config.pace_cap)")
    ap.add_argument("--pace_front_geo", action="store_true", help="the paced target as a front grown along the target's geodesics (config.pace_front_geo)")
    ap.add_argument("--pace_front_fill", action="store_true", help="with a front: held images assigned to revealed vacancies, one per target point (config.pace_front_fill)")
    ap.add_argument("--pace_front_thin", action="store_true", help="the front and its cap only on the sub-bulk part of the target (config.pace_front_thin)")
    ap.add_argument("--pace_front_dense", action="store_true", help="the front's fill at half the target point's own shell count (config.pace_front_dense)")
    ap.add_argument("--pace_coherent", action="store_true", help="the paced target keeps a material neighbourhood together: a particle ahead of its plan neighbours waits (config.pace_coherent)")
    ap.add_argument("--render_paced", action="store_true", help="the render channel targets the paced cloud's own silhouettes and shading (config.render_paced)")
    ap.add_argument("--pace_stream", action="store_true", help="the pace step scales with the fill of the stream one pace behind the particle on its ray (config.pace_stream)")
    ap.add_argument("--pace_support", action="store_true", help="the pace step scales with the density at the particle over half the target's own count there (config.pace_support)")
    ...
    ap.add_argument("--settle_pin_kkt_dry", action="store_true", help="with --settle_pin_kkt: record the evidence, release nothing")
    ...
    ap.add_argument("--h1_outside", action="store_true", help="the H^-1 term outside the physics core (config.h1_outside)")
    ap.add_argument("--h1_onset_pin", action="store_true", help="the H^-1 term from the pin's onset (config.h1_onset_pin)")
