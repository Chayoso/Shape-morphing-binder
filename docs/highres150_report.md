...
실험 사다리와 각 단계의 수치는 docs/experiments.md 2026-09-16 'mass-ejection ladder' 절에 있다.진단도구: scripts/probes/fragment_trace.py(이탈 입자의 소스 이웃 거리비 시계열), stray_census.py(끝 프레임 far 입자), selfprop_probe.py(단일 입자 자기추진 — 무시할 수준).이탈 입자는 소스 구의 바깥 10 % 껍질 입자(dragon 81 %, armadillo 100 %, bob 45 %)이고, 이웃 거리비가 run의 1/12 시점에 4.6–6, 1/4 시점에 11–14, 이후 평탄 — 초기 팽창 구간의 선두 입자들이 매 window 제어 clip(0.02) 한계만큼 이웃보다 앞서 밀려 떨어진 뒤 다시 돌아오지 않는다(한 셀 이상 떨어지면 MPM은 두 입자를 연결하지 않고, 매 commit의 소성 흡수가 되돌리는 응력을 지운다).메커니즘 사다리 — 고정 도메인/보정된 자동 도메인에서 유효한 판정메커니즘dragon 40k chamfer / silIoUfar &gt;0.5 wu판정per-particle + 재료 재결합 v5 (bonds, no-grad 경로 버그 수정 후) — 기준0.1199 / 0.901 (auto) · 0.1224 / 0.887 (fixed)83 · 132최선, 미해결Sobolev(H1) 하강 방향 (--grad_h1)0.1240 / 0.88297반증이웃 합의 소성 흡수 (--assim_consensus)0.1294 / 0.824219반증제어 basis 24³ (--control_grid 24)0.1324 / 0.931192이탈 악화격리 veto · 탈출 hinge · 속도 cap · 연속성 line search · bond spring · 재결합 v2–v4——이전 절 (모두 반증)보정 버그--domain auto(속도 작업)가 밀도 단위 보정의 기준 셀(0.5 wu)을 cfg에 전달하지 않아 유효 가중치가 ~3배 어긋났고, 그 사이에 나온 basis/consensus/Sobolev의 '재앙적' 결과와 첫 150k 배치는 모두 이 버그의 산물이었다(dragon 40k 비율 4.36e4 → 수정 후 1.41e4, 고정 도메인 1.45e4). 위 표는 수정 후 재실행값이다.구조적으로 남은 방향문헌상 입자 분리를 구성적으로 막는 정식화는 참조 배치에 연결성을 고정하는 total-Lagrangian MPM(de Vaucorbeil 등; 형상함수를 미변형 격자에서 평가해 이웃을 영구히 유지)과 입자 도메인을 셀 크기로 제한하는 CPDI 계열(Simulating Brittle Fracture with Material Points)이다. 대가는 위상 변화(A의 구멍, teapot 손잡이)를 만드는 바로 그 numerical fracture를 금지한다는 것 — 이산화가 정의하는 결합 범위를 가진 혼합 정식화가 열린 설계다.2026-09-16 밤 — 다섯 가설과 실험 (docs/experiments.md 같은 절)#가설근거 문헌실험 / 결과H1자유 표면에서 스텐실보다 미세한 제어장이 이탈을 만든다; 참조 연결성(TLMPM/A-ULMPM/CPDI)은 numerical fracture를 없앤다de Vaucorbeil 2020 TLMPM; Sadeghirad 2011 CPDI; Su 2022 A-ULMPM제어 해상도 변형(basis 24³)은 이미 반증(ejc2). 참조 연결성 정식화는 소성 흡수와 결합하면 이탈 대신 필라멘트를 만들 것으로 예상 — 미구현(엔진 수정)H2항복면 없는 비율 소성(η=0.5)이 표류를 몸체 유동과 똑같이 용서한다Stomakhin 2013; Klár 2016 Drucker–Prager표류율(2 %/window)이 몸체 유동율과 같아 항복 기준으로 분리 불가 — 실험 보류H3셀 합 밀도 손실은 빈 타깃 셀의 외톨이 입자에 최대 한계 이득을 준다; 수송(EMD/Sinkhorn) 손실은 질량을 흐름으로 옮긴다Feydy 2019 GeomLoss; Huang 2021 PlasticineLab PRT-EMD확인: dragon 40k에서 재부착 횟수 227 → 1(윈도당 한 번의 Sinkhorn 계획, 8192 샘플). 품질은 아직 뒤짐(chamfer 0.156 vs 0.119) — 32768 샘플·ε=(0.5 dx)² 변형 진행 중H4입자처럼 보이는 것은 스플랫 렌더의 산물; 흐린 밀도의 등밀도면이 하나의 물체를 보여준다Yu & Turk 2013; van der Laan 2009; conservative resampling(arXiv 2603.03860)확인: render_iso_video.py — 위 표면 비디오가 그것. 진짜 간격으로 렌더하면 bunny 귀가 희박(bulk의 절반 미만)하다는 물리 사실이 드러난다H5렌더 기울기가 물리 기울기와 직교해 일을 하지 않는다Liu 2019 SoftRas; Laine 2020 nvdiffrast; Kerbl 2023 3DGSphysics-only 쌍둥이 대비 렌더 채널이 silIoU +1.4~+4.6, chamfer −1~3 %(dragon/armadillo/bunny) — 피드백은 실재. Sobolev 평활(render_gs)은 변화 없음(0.966 → 0.970)
