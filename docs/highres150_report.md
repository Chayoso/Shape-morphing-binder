...
10개 타깃 150k 결과와 표면 비디오는 위 표와 예제 절, 3D 뷰어는 아래 절.Mass ejection: 원인은 초기 팽창 구간의 표면 선두 입자 + numerical fracture + 소성 흡수의 조합으로 좁혀졌고, 유효한 판정 기준으로 12개 구조적 메커니즘이 반증됐다. 현재 최선은 재료 재결합 v5(dragon far 274 → 83, chamfer 0.1325 → 0.1199). 남은 구조적 방향은 참조 연결성(TLMPM/CPDI)과 위상 변화의 절충.속도: 150k window당 6.8 → 2.3–2.9 s(무부하), 300 window 12–14 min. 10 min 미만은 P2G/G2P 수동 adjoint가 필요.부수 수정: (a) line search·commit 롤아웃의 bond 누락, (b) 자동 도메인의 보정 셀 누락, (c) heart 150k의 외부 merit 조기 동결은 (b)의 결과였다.
