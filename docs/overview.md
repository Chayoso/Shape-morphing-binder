...
**State as of 2026-09-23 03:00.** The user chose g41 after the surface analysis: RECIPE = RECIPE_V8 + `--layer_relax --pbr_denoised --layer_ctrl --sampler stratified --layer_gate_ot`, the catastrophe brake on the primary objective, videos tracked under the re-mesh policy of method.md §10.15 (addendum). The 19 g41 videos are re-rendered under it and the g41 page republished; the user judges from the videos next. Speed: the merit divergence warm-starts, videos reconstruct frames in parallel (`--prefetch`).
